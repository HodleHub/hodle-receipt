...
export type DecodedInvoice = ReturnType<typeof decode>
...
export async function getPubkeyFromSignature(decoded: DecodedBolt11) {
  const signature = decoded.sections.find(
    (section: Bolt11Section) => section.name === 'signature',
  )
  ...
  const prefix = decoded.sections
    .filter((section: Bolt11Section) => prefixSections.includes(section.name))
    .map((section: Bolt11Section) => {
  ...
  const separator = decoded.sections.find(
    (section: Bolt11Section) => section.name === 'separator',
  )?.letters
  ...
  const signingData =
    strToHex(prefix) + byteArrayToHexString(bech32To8BitArray(data))

  const hash = await crypto.subtle.digest(
    'SHA-256',
    hexToArrayBuffer(signingData),
  )
...
export function formatLong(text: string) {
  const long = text
    .replace(/\s+/g, '')
    .toUpperCase()
    .slice(Math.max(0, text.length - 16))
  return `0x${long}`
}
