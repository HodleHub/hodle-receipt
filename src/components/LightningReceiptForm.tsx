...
import { formatLong, verifyPaymentProof, getPubkeyFromSignature, type LightningReceipt } from '@/utils/lightning';
...
export default function LightningReceiptForm() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const proofParam = url.searchParams.get('proof') || url.searchParams.get('preimage');
  ...
  useEffect(() => {
    const decodeInvoice = async () => {
      ...
      try {
        ...
        const description =
          decoded.sections.find((section) => section.name === 'description')?.value ?? 'vazio';
        const paymentHash =
        ...
        if (formData.preimage) {
          ...
          const pubKey = await getPubkeyFromSignature(decoded);
    ...
    <Card className="w-full">
      <CardHeader className="text-center">
        ...
        <CardDescription className="text-xs">
          <p>
            A pré-imagem fornecida prova criptograficamente que a invoice especificada foi
            paga com sucesso.
          </p>
          <p>
            Saiba mais{' '}
                  ...
                  {decodedInvoice && (
                    <>
                      ...
                      <tr className="border-b">
                        ...
                        <td className="py-3 px-4 text-right">{decodedInvoice.description}</td>
                      </tr>
                      <tr className="border-b">
                        <td className="py-3 px-4 font-medium">Hash de Pagamento</td>
                        <td className="py-3 px-4 text-right flex items-center justify-end">
                          <span className="truncate max-w-[160px]">
                            {formatLong(decodedInvoice.paymentHash)}
                          </span>
                          <CopyButton title="hash de pagamento" value={decodedInvoice.paymentHash} />
                        </td>
                      </tr>
                      {payeePubKey && (
                        <tr className="border-b">
                          <td className="py-3 px-4 font-medium">Chave Pública do Recebedor</td>
                          <td className="py-3 px-4 text-right flex items-center justify-end">
                            <span className="truncate max-w-[160px]">{formatLong(payeePubKey)}</span>
                            <Link
